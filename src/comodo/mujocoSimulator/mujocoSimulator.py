...
class MujocoSimulator(Simulator): 
    ...
    def load_model(self, robot_model,  s, xyz_rpy, kv_motors = None, Im = None): 
        ...
        self.kv_motors = kv_motors if not(kv_motors is None) else np.zeros(self.robot_model.NDoF)
    ...
    def step_with_motors(self,n_step, torque): 
        ...
        for _ in range(n_step): 
            ...
            input = np.asarray([self.Im[item]*s_dot_dot[item] + self.kv_motors[item]*s_dot[item] + torque[item] for item in range(self.robot_model.NDoF)])
