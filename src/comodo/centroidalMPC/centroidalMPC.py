...
class CentroidalMPC(Planner): 
    ...
    def __init__(self, robot_model,step_length,frequency_ms = 100):
        ...
        self.contact_planner = FootPositionPlanner(robot_model=robot_model, dT=self.dT_in_seconds, step_length=step_length)
        ...
        self.contact_planner.set_scaling_parameters(scaling=scaling, scalingPos=scalingPos, scalingPosY = scalingPosY)
    ...
    def plan_trajectory(self):
        ...
        self.centroidal_mpc.set_reference_trajectory(self.com_traj, self.angular_mom_trak)
        self.centroidal_mpc.set_contact_phase_list(self.contact_phase_list)
        success = self.centroidal_mpc.advance()
        if(success):
            self.centroidal_dynamics.set_control_input((self.centroidal_mpc.get_output().contacts, np.zeros(6)))
    ...
    def intialize_mpc(self, mpc_parameters:MPCParameterTuning): 
        ...
        self.contact_0_handler.set_parameter_vector_float("corner_2", [-0.1,-0.05,0.0])
        self.contact_0_handler.set_parameter_vector_float("corner_3", [-0.1,0.05,0.0])
        self.contact_0_handler.set_parameter_vector_float("bounding_box_lower_limit", [0.0,0.0,0.0])
        self.contact_0_handler.set_parameter_vector_float("bounding_box_upper_limit", [0.0,0.0,0.0])
        ...
        self.contact_1_handler.set_parameter_vector_float("corner_2", [-0.1,-0.05,0.0])
        self.contact_1_handler.set_parameter_vector_float("corner_3", [-0.1,0.05,0.0])
        self.contact_1_handler.set_parameter_vector_float("bounding_box_lower_limit", [0.0,0.0,0.0])
        self.contact_1_handler.set_parameter_vector_float("bounding_box_upper_limit", [0.0,0.0,0.0])
    ...
    def define_test_com_traj(self, com0): 
        ...
        for item in vector_phase_list:
            if(len(item.active_contacts)== 2 and not(vector_phase_list.first_phase() is item) and not(vector_phase_list.last_phase() is item)):
            ...
            elif(len(item.active_contacts) == 2 and (vector_phase_list.last_phase() is item)): 
        ...
        for i in range(tempInt):
            ...
            com_temp = np.zeros(3)
            com_spline.evaluate_point(i*self.dT_in_seconds,com_temp,velocity, acceleration)
    ...
    def plot_3d_foot(self):
        ...
        for item in contact_left.corners:
            self.left_foot_fig.quiver(item.position[0],item.position[1],item.position[2],item.force[0],item.force[1],item.force[2])
        for item in contact_right.corners:
            self.right_foot_fig.quiver(item.position[0],item.position[1],item.position[2],item.force[0],item.force[1],item.force[2])
    ...
    def initialize_centroidal_integrator(self, s,s_dot,H_b,w_b,t ):
        self.centroidal_integrator = blf.continuous_dynamical_system.CentroidalDynamicsForwardEulerIntegrator()
